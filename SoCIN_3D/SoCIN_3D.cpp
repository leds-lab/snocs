      ...
      w_Z_VC_SELECTOR_TO_DOWN("w_Z_VC_SELECTOR_TO_DOWN")
{
    ...
    for( unsigned char x = 0; x < numberOfXWires; x++ ) {
        ...
        // Tm for WEST link
        TrafficMeter* u_TM_LEFT = new TrafficMeter(strXWMeter,WORK_DIR,strXWMeter,INoC::TT_Orthogonal3D,false);
        // System signals
        ...
        // Tm for EAST link
        TrafficMeter* u_TM_RIGHT = new TrafficMeter(strXEMeter,WORK_DIR,strXEMeter,INoC::TT_Orthogonal3D,false);
        // System signals
    ...
    for( unsigned char y = 0; y < numberOfYWires; y++ ) {
        ...
        // Tm for NORTH link
        TrafficMeter* u_TM_NORTH = new TrafficMeter(strYNMeter,WORK_DIR,strYNMeter,INoC::TT_Orthogonal3D,false);
        // System signals
        ...
        // Tm for SOUTH link
        TrafficMeter* u_TM_SOUTH = new TrafficMeter(strYSMeter,WORK_DIR,strYSMeter,INoC::TT_Orthogonal3D,false);
        // System signals
    ...
    for( unsigned char z = 0; z < numberOfZWires; z++ ) {
        char strZUMeter[20];
        sprintf(strZUMeter,"int_z_u_%u",z);
        // Tm for UP link
        TrafficMeter* u_TM_UP = new TrafficMeter(strZUMeter,WORK_DIR,strZUMeter,INoC::TT_Orthogonal3D,false);
        // System signals
        u_TM_UP->i_CLK(i_CLK);
        u_TM_UP->i_RST(i_RST);
        u_TM_UP->i_CLK_CYCLES(i_CLK_GLOBAL);
        u_TM_UP->i_EOS(i_EOS);
        // Link signals
        u_TM_UP->i_DATA(w_Z_DATA_TO_UP[z]);
        u_TM_UP->i_VALID(w_Z_VALID_TO_UP[z]);
        u_TM_UP->i_RETURN(w_Z_RETURN_TO_UP[z]);
        if( NUM_VC > 1 ) {
            u_TM_UP->i_VC_SEL(w_Z_VC_SELECTOR_TO_UP[z]);
        }

        char strZDMeter[20];
        sprintf(strZDMeter,"int_z_d_%u",z);
        // Tm for DOWN link
        TrafficMeter* u_TM_DOWN = new TrafficMeter(strZDMeter,WORK_DIR,strZDMeter,INoC::TT_Orthogonal3D,false);
        // System signals
        u_TM_DOWN->i_CLK(i_CLK);
        u_TM_DOWN->i_RST(i_RST);
        u_TM_DOWN->i_CLK_CYCLES(i_CLK_GLOBAL);
        u_TM_DOWN->i_EOS(i_EOS);
        // Link signals
        u_TM_DOWN->i_DATA(w_Z_DATA_TO_DOWN[z]);
        u_TM_DOWN->i_VALID(w_Z_VALID_TO_DOWN[z]);
        u_TM_DOWN->i_RETURN(w_Z_RETURN_TO_DOWN[z]);
        if( NUM_VC > 1 ) {
            u_TM_DOWN->i_VC_SEL(w_Z_VC_SELECTOR_TO_DOWN[z]);
        }
    }
